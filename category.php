...
if (!$smarty->is_cached($dwt_name.'.dwt', $cache_id))
{
    ...
    //分类层级 id by luojing 2015-2-10
    $cat_arr = get_parent_cats($cat_id);
    $cat_arr_true = array_reverse($cat_arr);
    $cat_level = array(0=>0,1=>0,2=>0);
    foreach($cat_arr_true as $i=>$v){
        $cat_level[$i] = $v['cat_id'];
    }  
    $smarty->assign('cat_level',  $cat_level);

    //assign_template('c', array($cat_id));
    assign_template('c', array($cat_level[0],$cat_level[1],$cat_level[2]));
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.comments_number,g.sales_volume,g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
