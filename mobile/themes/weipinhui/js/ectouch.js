...
//商品飞入购物车效果
$(function(){
	var bodyW=$(window).width(),bodyH=$(window).height();
	var black_cart=$(".black_cart");
	var div=$("<img src='"+GOODS_IMAGE+"'class='box02'/>");
	
	$("#onclick_cart").click(function(){
		var black_cartL=black_cart.offset().left,black_cartT=black_cart.offset().top-$(window).scrollTop();	
		div.insertAfter(this);
		var box02=$(".box02");
		var imgW=box02.width(),imgH=box02.height();
		var imgL=(bodyW)/2;
		var imgT=(bodyH-imgH)/2;
		div.css({left:imgL,top:imgT,opacity:1,width:"9em"});
		div.stop(true,false).animate({left:black_cartL,top:black_cartT,width:"2em"},1000,function(){
			div.stop(true,false).animate({"opacity":0},500,function(){
				$(".AddCartSuccess").fadeIn(500);
	...
	$(window).resize(function(){
		bodyW=$(window).width(),bodyH=$(window).height();
	})
});
