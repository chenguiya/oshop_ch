...
function insert_ads($arr)
{
    ...
    foreach ($res AS $row)
    {
		...
		if($row['position_name'] == "首页名品特卖广告3-10")
		{
			$src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
							DATA_DIR . "/afficheimg/$row[ad_code]" : $row['ad_code'];
							
			$ads[] = '<li>
						<p class="s_time">剩余:<i class="s_time_icon2" showday="show" value="'.$row['end_time'].'"></i></p>
						<a href="affiche.php?ad_id=$row[ad_id]&amp;uri='.urlencode($row['ad_link']). '" target="_blank"><img src="'.$src.'" width="' .$row['ad_width']. '" height="'.$row[ad_height].'" border="0" /></a>
					 </li>';
