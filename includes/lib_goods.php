...
function get_child_tree($tree_id = 0)
{
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        ...
        $num = 1;  // add by zhangjh 2015-09-16 加记数 num
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
 ...
 */
function get_products_info_extend($goods_id, $spec_goods_attr_id)
{   
    $return_array = array();
    $list_product = array();
    $product_number = null;
    ...
    {   
        $ANDSQL = '';
        foreach($spec_goods_attr_id as $attr_id){
            $ANDSQL .= "AND goods_attr like '%$attr_id%'";
        }       
        $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('products'). " WHERE goods_id = '$goods_id' $ANDSQL";
        $list_product = $GLOBALS['db']->getAll($sql);
    }
