...
function assign_comment($id, $type, $page = 1)
{
    ...
    $sql = 'SELECT c.*,g.goods_name FROM ' . $GLOBALS['ecs']->table('comment') .' AS c LEFT JOIN '.$GLOBALS['ecs']->table('goods').
            " AS g ON c.id_value = g.goods_id WHERE c.id_value = '$id' AND comment_type = '$type' AND c.status = 1 AND c.parent_id = 0".
