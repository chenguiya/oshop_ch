...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    else
    {
        ...
        //联动库存 by luojing 2015/01/27
        $res['pdt_num'] = 0;
        $products = get_products_info($goods_id, $attr_id);    
        if(!empty($products))
        {
            $res['pdt_num'] = $products['product_number'];
        }
    }
...
/*------------------------------------------------------ */
//-- 改变属性、数量时重新计算商品价格- 推展 by luojing 2015/02/5 ***********************
/*------------------------------------------------------ */

if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price_extend')
{
    include('includes/cls_json.php');
    ...
    else
    {
        ...
        //联动库存 by luojing 2015/02/6
        $res['pdt_num'] = 0;
        $product_number = get_products_info_extend($goods_id, $attr_id); 
        
        if(!is_null($product_number))
        {
            $res['pdt_num'] = $product_number;
        }
    }
